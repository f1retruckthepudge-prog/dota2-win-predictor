...
## Простейшая модель для прогноза победы (без ML)

На основе трёх факторов (активное золото, темп опыта, токсичность) создано простое правило:

- `gold_other > 0` → +1  
- `xp_growth > 0` → +1  
- `toxic_diff <= 0` (Radiant не токсичнее Dire) → +1  

**Сумма:**  
- 2–3 → победа Radiant вероятна (>65%)  
- 0–1 → поражение вероятно (<35%)

**Проверка на 5 случайных матчах:** 4 из 5 правильных. Модель не заменяет ML, но показывает, как инсайты превращаются в решение.
